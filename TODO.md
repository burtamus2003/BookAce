...
- [x] Decided not to pursue Gmail BIMI display — would require purchasing a Verified Mark Certificate, not worth it right now (closes #3). Correction: Apple Mail was confirmed by actual testing to *not* render the icon either, despite the original claim here — no client is confirmed working without a VMC at this point
